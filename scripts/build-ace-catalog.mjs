...
// parameter IDs, types and combo items, and each expression's name (the form
// written inside parameter strings), parameter types and whether it takes a
...
/** One expression reduced to what an expression string reveals: its name, parameter types, variadic flag. */
function expression(ace) {
  const entry = { name: ace.expressionName ?? ace.id, params: params(ace).map(p => p.type) };
  if (ace.isVariadicParameters) entry.variadic = true;
  if (typeof ace.returnType === 'string') entry.returns = ace.returnType;
  return entry;
}

/** An addon's categories flattened to { conditions: {id: params}, actions: {id: params}, expressions: {id: entry} }. */
function flatten(addon) {
  const result = { conditions: {}, actions: {}, expressions: {} };
  for (const category of Object.values(addon)) {
    ...
    for (const ace of category.expressions ?? []) result.expressions[ace.id] = expression(ace);
  }
  return result;
}
 ...
 * The registering function's minified name tells condition, action and
 * expression apart; it is identified from IDs whose kind is certain rather
 * than hard-coded, because it changes between releases.
 */
function commonAces(mainJs) {
  const found = [];
  const re = /([A-Za-z_$][\w$]*)\(\{id:"([a-z0-9-]+)",(?:c2id|scriptName|expressionName):/g;
  let m;
  ...
  const kindOf = { 'compare-instance-variable': 'conditions', 'set-instvar-value': 'actions', 'uid': 'expressions' };
  const fnKind = {};
  for (const { fn, ace } of found) if (kindOf[ace.id]) fnKind[fn] = kindOf[ace.id];
  if (Object.values(fnKind).sort().join() !== 'actions,conditions,expressions') {
    throw new Error(`Could not tell the condition, action and expression registrations apart: ${JSON.stringify(fnKind)}`);
  }
  const result = { conditions: {}, actions: {}, expressions: {} };
  for (const { fn, ace } of found) {
    const kind = fnKind[fn];
    if (kind === 'expressions') result.expressions[ace.id] = expression(ace);
    else if (kind) result[kind][ace.id] = params(ace);
  }
  return result;
}
...
const countExpressions = group => Object.values(group).reduce((n, a) => n + Object.keys(a.expressions).length, 0);
const header = `// Generated by scripts/build-ace-catalog.mjs from Construct ${release}. Do not edit by hand.
// ${Object.keys(catalog.plugins).length} plugins and ${Object.keys(catalog.behaviors).length} behaviors with ${count(catalog.plugins) + count(catalog.behaviors)} conditions and actions and ${countExpressions(catalog.plugins) + countExpressions(catalog.behaviors)} expressions,
// plus ${Object.keys(catalog.common.conditions).length + Object.keys(catalog.common.actions).length} common ACEs and ${Object.keys(catalog.common.expressions).length} common expressions.
