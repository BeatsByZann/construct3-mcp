...
- Event subscriptions (upstream issue 11): `subscribe_events`, `read_events` and `unsubscribe_events`, on three new bridge commands. The bridge keeps a bounded FIFO buffer per subscription (1 to 1000 events, oldest dropped when full) and fills it from documented runtime state on every tick: each change of one global variable (`Object.is`, no initial event), each change of the current layout, and each custom event the game emits with `globalThis.__c3bridge.emit(name, data)`, optionally filtered by name. Events carry type, name, value, previousValue, timestamp and tick. Queued commands run before the tick's comparison, so a subscription made on one tick observes from the next. A generic "signal" subscription is not offered: no documented Construct interface observes every event-sheet signal or plugin trigger. The bridge header no longer claims a fetch endpoint. 185 tools.

- File stamps and a change journal for runs that share a project with the editor. The server remembers the size and modification time of every file it reads or writes; a call that reads a file Construct saved since then says so in its result and works from the new content, and a write made from a stale bulk read is refused until `reload_project` (new), which re-reads the project and reports which files had changed on disk. Every call's result ends with the files it changed (`Changed N file(s): ...`); `list_changes` (new) shows what recent calls wrote, created, deleted or moved with the `.bak` each write left, and `revert_last_change` (new) undoes the most recent such call from those backups, refusing when a later call touched the same files. 182 tools.

- Expression checking: the value of every expression-typed parameter, and every argument of a function or custom action call, is parsed as a Construct expression and its names resolved against the project and the catalogue, which now carries the 1,271 plugin and behavior expressions and 47 common expressions Construct r495.2 defines (and a third-party addon's, once loaded). `validate_project` reports `expression-syntax`, `expression-unknown-object`, `expression-unknown-member`, `expression-unknown-function`, `expression-unknown-name` and `expression-argument-count` warnings (18 checks now run), and `add_event_block`, `update_event_block`, `update_event_block_action`, `replace_object_in_events` and `replace_in_expressions` warn about what they write or change. Names match without case; a family's instance variables reach its members; `Self` is the ACE's own object; the functions namespace follows the project's `functionsName`. Checked against C3-ACE's 4,757 parameter values: 0 problems.
...
- ACE validation: conditions and actions of built-in plugins and behaviors are checked against the definitions Construct r495.2 ships to its own editor (1,916 plugin and behavior ACEs plus 84 common ones). `add_event_block`, `update_event_block` (for what it adds or changes), `update_event_block_action`, `replace_object_in_events` and `replace_in_expressions` (for what they change) warn about an unknown ACE, an unknown or missing parameter, and a combo value that is not one of its choices; `validate_project` reports the same across the project as `ace-*` warnings and now runs 15 checks (16 with the legacy event-key check below). Third-party addons are skipped. The catalogue is generated by `scripts/build-ace-catalog.mjs`. Checked against 8,191 built-in ACEs in C3-ACE and three reference packages saved by Construct: 0 problems.
- Single-file projects: pass a `.c3p` path to serve it. The archive is unpacked into a working folder, and after every tool call that changed a file it is written back (built in memory, read back, written beside the original and renamed over it; the first write keeps `<name>.c3p.bak`), with a line in the tool result. `open_project` switches a running server to another project folder, `.c3proj` or `.c3p` (waiting for other calls to finish, writing the archive it leaves, and keeping the current project if the new one cannot be opened), and `get_open_project` reports which project is served. A write is refused, and the working folder kept, when something else changed the archive since the server opened or last wrote it. The new dependency-free zip reader reads Construct's own archives, which are ZIP64 with backslash paths, and refuses encrypted, multi-disk and unsafe-path entries. `writeZip` takes `compress` for DEFLATE and uses zlib's native CRC-32 when Node has it.
- Bulk instance tools: `add_instances_to_layout`, `update_instances`, `move_instances` and `delete_instances_from_layout` apply up to 500 edits to one layout in one call, in list order. Every item is checked and applied in memory first; if any item fails, the error names it by index and nothing is written, and otherwise the layout file is written once. Each takes `dryRun`. The single and bulk tools now share one implementation per edit, and a test checks that a bulk call leaves the layout exactly as the same single-tool calls do.
...
- Timeline audio tracks keep the Sounds or Music subfolder in `audioProjectFilePath` (`media/Sfx/Blip.webm`, from the second W90 round trip) and require their first keyframe at time 0, which is where r495.2 moves it.
...
- `revert_last_change` could restore the wrong state and report success. `create_timeline`, `delete_timeline`, `create_ease`, `delete_ease`, `create_flowchart`, `delete_flowchart` and several other tools wrote `project.c3proj` (and their own files) outside the change journal while still refreshing `project.c3proj.bak`, so reverting the call before them restored that newer backup: after `update_project_properties` then `create_timeline`, a revert kept the property, dropped the timeline's registration, left its file, and named `update_project_properties` as reverted. Every tool that writes project files now records the write (timelines, eases, tracks, flowcharts, containers, tilemap brushes, addon registration, script and project files, the runtime bridge and `main.js`, Project Bar moves and duplicates, replaced images), so those calls are listed and revertible themselves; a file written twice in one call keeps one backup holding its content from before the call (a new plugin's registration and the object's own entry used to leave the intermediate state in `project.c3proj.bak`); and at the end of each call the content hash of every changed file and backup is kept, and a revert is refused, restoring nothing, when any of them changed since. `register_addon` and `unregister_addon` now leave a `.bak`.
- `create_timeline`, `delete_timeline`, `create_ease` and `delete_ease` now also write `project.c3proj` in the r495.2 save shape, like every other tool; they were missed when that change was made.

- A `.c3p` project's working folder is compared with its last write by each file's size and modification time, so a second write of the same size inside one time-stamp tick looked unchanged and was not written back (the archive test for a conflict after an earlier save failed now and then for this reason). A file modified within the last 100 ms now carries a content hash in its stamp, the way git guards the same race.
- `delete_layer` finds a layer at any nesting level and counts the instances on its sub-layers. It looked only at top-level layers and only at a layer's own instances, so deleting a parent layer with no instances of its own (`hud_DevToolOverlay` in C3-ACE, whose sub-layers hold 25) succeeded without `force` and removed every sub-layer and instance beneath it, and a sub-layer could not be deleted by name at all. Without `force` the call is now refused when the layer holds any instance at any depth or has any sub-layer; the result names the sub-layers.
- `rename_object_type`, `rename_family`, `rename_layout` and `rename_event_sheet` compare names without case, because they are file names. Renaming `Player` to `enemy` while `Enemy` existed passed the duplicate check and, on Windows, wrote Player's data over `Enemy.json`, leaving `project.c3proj` with two names for one file. A rename that changes only the case of the same name is refused as well: the rename writes the new file and then deletes the old, and on a case-insensitive disk those are one file, so `Main` to `MAIN` deleted the sheet.
- `rename_event_variable` rewrites a local variable only inside the container that declares it. Two sibling groups each declaring a local `Score`: renaming one also rewrote the other group's uses, leaving that group reading a name nothing declares.
- `delete_keyframe` refuses to delete an audio track's keyframe at time 0, which `add_audio_track` requires because Construct moves a keyframe there on load; deleting it left a track the editor silently rewrote.
- The frame tools take each image file's extension from the frame's `fileType`. They assumed `.png`, so for a GIF-backed animation (C3-ACE has 30 such frames) `reorder_frames` and `reverse_frames` renumbered the JSON and moved no files, `duplicate_frame` created a frame with no file, `delete_frame_from_animation` and `add_frame_to_animation` shifted the wrong files, and `rename_animation` left the GIFs under the old name. `replace_sprite_image` on a GIF frame now records the PNG it writes and removes the old `.gif`.
- `update_function` writes the caller sheets first and the definition's sheet last, and a write that fails part way names the sheets written and not written; it wrote the definition first, so a later failure left callers on a name that no longer existed and the error named no file. Re-running the same call resumes.
- `add_event_block` and `move_event_block` refuse an else block with nothing before it, or after a group, comment, variable or include: Construct's editor only offers "else" on a block that follows another block, and what it does with the other shape was never verified. Moving a block away from in front of an else warns.
- `validate_project`'s duplicate-UID and duplicate-SID checks walk sub-layers; a duplicate on a sub-layer was not reported, although the ID generator already avoided them.
- `replace_object_in_events` swapped an event whose behavior the replacement has under the same name but as another kind (a `Move` that is Platform on one object and Bullet on the other), leaving the event addressing the wrong behavior. Such an event is now left alone and reported.
- `rename_animation` moves the animation's frame image files to the new name. It renamed the animation in the object type only, so every frame lost its image: after renaming `Animation 1` to `Run`, the files stayed `<name>-animation 1-NNN.png` where Construct looks for `<name>-run-NNN.png`, and `validate_project` reported each frame missing. A rename that changes only the case moves nothing, since both names map to the same lowercase file. A new name that another animation has in a different case is refused, because the two would share every frame file. A frame with a file under both names is refused before anything moves rather than overwriting one; a frame with a file only under the new name keeps it, which repairs a rename stranded by an earlier version. The moves are undone if the object type cannot be written. Test fixtures that still wrote or read frame files under mixed-case names now use the lowercase names the tools write; they passed only on filesystems that ignore case.
- Sprite frame files are written under an all-lowercase name, the animation part included, as Construct saves them. `getImageFileName` lowercased the object name but kept the animation's case, so a new Sprite's first frame was written as `<name>-Animation 1-000.png` where Construct r495.2 writes `<name>-animation 1-000.png`, measured by saving an editor-made Sprite. Windows lookups ignore case, so this never showed there; on a case-sensitive filesystem the tools and Construct disagreed about which file a frame is. Every tool that writes, moves, copies or deletes frame files goes through this helper. A mixed-case file an earlier version wrote is still found on Windows, and is renamed to lowercase the next time a frame tool moves it.
...
- `add_instance_to_layout` wrote a centered world origin for objects whose `origin` property says otherwise (a Tiled Background came back from the r495.2 editor with 0,0), and accepted Sprite origins the editor replaces with the frame origin. It now derives the origin from the property or the Sprite frame, maps `originX`/`originY` to the property, and refuses a Sprite origin that differs from the frame.
